...
type QuestionTemplate616 struct {
    Templates []IQuestionTemplate
}

var (
    qt616     *QuestionTemplate616
    qt616Once sync.Once
)


func GetQuestionTemplate616() *QuestionTemplate616 {
    qt616Once.Do(func() {
        qt616 = &QuestionTemplate616{
            Templates: []IQuestionTemplate{new(QuestionTemplate616_1),new(QuestionTemplate616_3),new(QuestionTemplate616_4)},
        }
    })
    return qt616
}
...
func GenerateMath616Questions(subtype string, count int) []interface{} {
	...
	tplContainer := GetQuestionTemplate616()
    ...
    for i := 0; i < count; i++ {
        ...
        switch subtype {
        case "type1":
			tpl := tplContainer.Templates[0]
            
			tpl.randomData();
			questionText = tpl.generateQuestion()

        case "type2":
            tpl := tplContainer.Templates[1]
		    tpl.randomData();
			questionText = tpl.generateQuestion()

        case "type3":
            tpl := tplContainer.Templates[2]
		    tpl.randomData();
			questionText = tpl.generateQuestion()
